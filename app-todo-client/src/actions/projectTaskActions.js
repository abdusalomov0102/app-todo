...
export const getBacklog = () => async dispatch => {
    const res = await axios.get("http://localhost:8082/api/board")
    dispatch({
...
export const deleteProjectTask = id => async dispatch => {
    if (window.confirm(`You are deleting project task ${id}, this action cannot be undone!`)) {
        await axios.delete(`http://localhost:8082/api/board/${id}`);
        dispatch({
...
export const getProjectTask = (id, history) => async dispatch => {
    try {
        const res = await axios.get(`http://localhost:8082/api/board/${id}`)
        dispatch({
